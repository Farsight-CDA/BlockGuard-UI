...
import { SdlValidationError } from './sdl-validation-error';
...
export class SDL {
	...
	static validateGPU(name: string, gpu: v3ResourceGPU | undefined) {
		if (gpu) {
			...
			if (
				units > 0 &&
				!GPU_SUPPORTED_VENDORS.some(
					(vendor) => vendor in (gpu.attributes?.vendor || {})
				)
			) {
				throw new Error(
					`GPU must be one of the supported vendors (${GPU_SUPPORTED_VENDORS.join(',')}).`
				);
			}
			...
			if (
				units > 0 &&
				!!gpu.attributes?.vendor[vendor] &&
				!Array.isArray(gpu.attributes.vendor[vendor])
			) {
				throw new Error(
			...
			if (
				...
				Object.values(gpu.attributes?.vendor || {}).some((models) =>
					models?.some(
						(model) =>
							model.interface &&
							!GPU_SUPPORTED_INTERFACES.includes(model.interface)
					)
				)
			) {
				throw new Error(
	...
	static validateStorage(
		name: string,
		storage?: v2ResourceStorage | v2ResourceStorageArray
	) {
		if (!storage) {
			...
			if (storage.attributes) {
				for (const [key, value] of Object.entries(storage.attributes)) {
					if (
						...
						throw new Error(
							"Storage attribute 'ram' must have 'persistent' set to 'false' or not defined for service " +
								name
						);
					}
				}
	...
	private validate() {
		...
		Object.entries(v3data.profiles.compute || {}).forEach(
			([name, { resources }]) => {
				if ('gpu' in resources) {
	...
	private validateEndpoints() {
		...
		Object.keys(this.data.endpoints).forEach((endpointName) => {
			...
			SdlValidationError.assert(
				this.ENDPOINT_NAME_VALIDATION_REGEX.test(endpointName),
				`Endpoint named "${endpointName}" is not a valid name.`
			);
			SdlValidationError.assert(
				!!endpoint.kind,
				`Endpoint named "${endpointName}" has no kind.`
			);
			SdlValidationError.assert(
				endpoint.kind === this.ENDPOINT_KIND_IP,
				`Endpoint named "${endpointName}" has an unknown kind "${endpoint.kind}".`
			);
		});
	}
	...
	private validateCredentials(serviceName: string) {
		...
		if (credentials) {
			const credentialsKeys: (keyof v2ServiceImageCredentials)[] = [
				'host',
				'username',
				'password'
			];
			credentialsKeys.forEach((key) => {
				SdlValidationError.assert(
					credentials[key]?.trim().length,
					`service "${serviceName}" credentials missing "${key}"`
				);
			});
		}
	}
	...
	private validateDeploymentWithRelations(serviceName: string) {
		...
		SdlValidationError.assert(
			deployment,
			`Service "${serviceName}" is not defined in the "deployment" section.`
		);

		Object.keys(this.data.deployment[serviceName]).forEach((deploymentName) => {
			const serviceDeployment =
				this.data.deployment[serviceName][deploymentName];
			...
			SdlValidationError.assert(
				infra,
				`The placement "${deploymentName}" is not defined in the "placement" section.`
			);
			SdlValidationError.assert(
		...
		this.data.services[serviceName].expose?.forEach((expose) => {
			...
			expose.to?.forEach((to) => {
				if (to.ip?.length > 0) {
					SdlValidationError.assert(
						to.global,
						`Error on "${serviceName}", if an IP is declared, the directive must be declared as global.`
					);

					if (!this.data.endpoints?.[to.ip]) {
						throw new SdlValidationError(
	...
	private validateEndpointsUtility() {
		if (this.data.endpoints) {
			Object.keys(this.data.endpoints).forEach((endpoint) => {
				SdlValidationError.assert(
					this.endpointsUsed.has(endpoint),
	...
	deploymentsByPlacement(placement: string) {
		...
		return Object.entries(deployments as object).filter(({ 1: deployment }) =>
			Object.prototype.hasOwnProperty.call(deployment, placement)
		);
	}
	...
	serviceResourceStorage(
		resource: v2ResourceStorageArray | v2ResourceStorage,
		asString: boolean
	) {
		const key = asString ? 'quantity' : 'size';
		...
		return storage.map((storage) =>
			storage.attributes
				? {
						...
						attributes: this.serviceResourceStorageAttributes(
							storage.attributes
						)
					}
				: {
	...
	serviceResourceStorageAttributes(attributes?: v2StorageAttributes) {
		...
		const pairs = Object.keys(attributes).map((key) => ({
			key,
	...
	v2ServiceResourceEndpoints(service: v2Service) {
		const endpointSequenceNumbers = this.computeEndpointSequenceNumbers(
			this.data
		);
	...
	): v2ManifestServiceParams | undefined {
		return {
	...
	v3ManifestService(
		...
		return {
			...
			args: service.args || null,
			env: service.env || null,
			resources: this.serviceResourcesBeta3(
				id,
				profile as v3ProfileCompute,
				service,
				asString
			),
			count: deployment[placement].count,
	...
	v2Manifest(asString: boolean = false): v2Manifest {
		return Object.keys(this.placements()).map((name) => ({
			Name: name,
			Services: this.deploymentsByPlacement(name).map(([service]) =>
				this.v2ManifestService(name, service, asString)
			)
		}));
	}
	...
	v3Manifest(asString: boolean = false): v3Manifest {
		...
		return Object.keys(this.placements()).map((name, pIdx) => ({
			...
			services: this.deploymentsByPlacement(name)
				...
				.map(([service], idx) =>
					this.v3ManifestService(serviceId(pIdx, idx), name, service, asString)
				)
		}));
	}
	...
	resourceUnitMemory(computeResources: v2ComputeResources, asString: boolean) {
		...
		return {
			quantity: {
				val: this.resourceValue(
					convertResourceString(computeResources.memory.size),
					asString
				)
			},
			attributes:
	...
	groupResourceUnits(
		resource: v2ComputeResources | undefined,
		asString: boolean
	) {
		if (!resource) return {};
		...
		if (this.version === 'beta3') {
			units.gpu = this.resourceUnitGpu(
				resource as v3ComputeResources,
				asString
			);
		}
	...
	exposeShouldBeIngress(expose: {
		...
		const externalPort =
			expose.externalPort === 0 ? expose.port : expose.externalPort;
	...
	v3Groups() {
		...
		>();
		const services = Object.entries(this.data.services).sort(([a], [b]) =>
			a.localeCompare(b)
		);

		for (const [svcName, service] of services) {
			for (const [placementName, svcdepl] of Object.entries(
				this.data.deployment[svcName]
			)) {
				...
				if (!location) {
					...
					const resID =
						group.dgroup.resources.length > 0
					...
					group.boundComputes[placementName][svcdepl.profile] =
						group.dgroup.resources.length - 1;
				} else {
		...
		return names
			...
			.map((group) =>
				group ? (group.dgroup as typeof group.dgroup) : {}
			) as Array<v3DeploymentGroup>;
	}
			...
			Object.keys(depl).forEach((placementName) => {
				...
				if (!group) {
					...
					if (group.requirements.attributes) {
						group.requirements.attributes = group.requirements.attributes.sort(
