...
<dialog bind:this={dialogElement} class="p-4 shadow-2xl rounded-lg bg-slate-900" on:click={dialogClickHandler} on:keydown={() => {}}>
    ...
    <div class="flex flex-col items-center gap-4" transition:scale={{ duration: 200, delay: 0 }} on:introstart on:outroend>
        ...
        <span>
            <input disabled={generatedMnemonics == null} id="copied" type="checkbox" bind:checked={hasSavedMnemonics}>
            <label for="copied">I have saved the mnemonics</label>
        </span>

        <button disabled={!hasSavedMnemonics} class="px-2 py-1" 
            class:bg-green-500={hasSavedMnemonics} class:bg-gray-800={!hasSavedMnemonics}
